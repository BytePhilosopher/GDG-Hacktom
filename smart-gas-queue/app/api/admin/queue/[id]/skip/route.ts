...
export async function PATCH(
  ...
  try {
    ...
    const maxPosition = maxRow?.position ?? queue.position;

    await adminClient.rpc('skip_queue_entry', {
      p_queue_id: id, p_station_id: profile.station_id,
      p_old_position: queue.position, p_max_position: maxPosition,
    });
    await adminClient.from('queue_history').insert({
      queue_id: id, station_id: profile.station_id,
      driver_id: queue.driver_id, action: 'skipped', performed_by: user.id,
    });
