...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (authError) {
      ...
      console.error('[auth/register] auth', authError);
    ...
    const { error: profileError } = await adminClient
      .from('profiles')
      .upsert({ id: userId, full_name: fullName, phone, role: 'driver' }, { onConflict: 'id' });
    ...
    if (vehicleError) {
      ...
      await adminClient.auth.admin.deleteUser(userId);
      console.error('[auth/register] vehicle', vehicleError);
      return NextResponse.json(
        { error: 'Could not save your vehicle details. Please try again.' },
        { status: 500 }
      );
    }
